...
public class Controllers : MonoBehaviour
{
    ...
    void Update()
    {
        numHandles = PS4Input.PadGetUsersPadHandles2(maxControllers, handles); // get details of connected controllers
    ...
    {
        int playerColor = PS4Input.PadGetUsersDetails(playerId).color;
        ...
        GUI.Label(internalDisplayArea,
            ...
            "\nPlayer ID: " + playerId +
            "\nColor: " + playerColor +
            "\nUser ID: " + PS4Input.PadGetUsersDetails(playerId).userId +
            "\nUser Name: " + PS4Input.PadGetUsersDetails(playerId).userName +
            "\nStatus: " + PS4Input.PadGetUsersDetails(playerId).status +
            "\nLast Orientation: " + PS4Input.GetLastOrientation(playerId) +
            "\nLast Gyro: " + PS4Input.GetLastGyro(playerId) +
            "\nLast Acceleration: " + PS4Input.GetLastAcceleration(playerId) +
            "\n" +
            // buttons
    ...
    private void UpdateJoystickRepresentations()
    {
        for (int i = 0; i < numHandles; i++)
        {
            ...
            if (PS4Input.PadIsConnected(i) == true)
            {
                ...
                currentJoystick.transform.eulerAngles = new Vector3(-PS4Input.GetLastOrientation(i).x, PS4Input.GetLastOrientation(i).y, PS4Input.GetLastOrientation(i).z) * 100;
            }
